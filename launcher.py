...
def _ensure_port_free_or_die() -> None:
    ...
    try:
        ...
        msg = (
            ...
            f"PCC requires port {PORT} for single-instance detection. "
...
def _activate_existing_window() -> bool:
    ...
    if sys.platform == "darwin":
        ...
        try:
            ...
            subprocess.run(
                ["osascript", "-e",
                 ...
                 'set frontmost of first process whose name is "PCC" to true'],
                capture_output=True, timeout=3,
            )
            return True
        except Exception:
        ...
        try:
            ...
            hwnd = user32.FindWindowW(None, "PCC")
            if hwnd:
    ...
    if sys.platform == "win32":
        try:
            ...
            ctypes.windll.user32.MessageBoxW(
                0,
                f"PCC's built-in window could not start "
...
def main():
    ...
    if is_already_running():
        if _activate_existing_window():
            print(f"PCC already running — activated existing window.")
            return
        ...
        print(f"PCC already running → {URL}")
        webbrowser.open(URL)
        return

    print(f"Starting PCC on {URL}...")
