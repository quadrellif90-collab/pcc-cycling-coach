# PPC v4.4.0 — Presentazione di rilascio

**Programming Cycling Coach** · Build del 2026-07-25 · Installer Windows + DMG macOS

---

## In sintesi

PPC 4.4.0 risolve tre irritazioni vecchie di mesi e rende il motore di
...
nessun server PPC. L'integrazione con intervals.icu è un *optional* che colleghi
tu.
...
- Il token **sopravvive ai riavvii** → apri PPC e sei già connesso.
...
- Build EXE/installer allineati (scipy 1.18, percorsi `dist/PPC`).
- **Installer Windows** incluso: `PPC-Setup-4.4.0.exe` installa con scorciatoia
...
|---|---|---|
| Windows | `PPC-Setup-4.4.0.exe` | Installer NSIS — doppio clic, Next/Next. Dati utente in `%USERPROFILE%\.domestique\` (non toccati da installazione/aggiornamento). |
| macOS | `PPC.dmg` | Trascina in Applicazioni. |
...
Questa release consolida la *base*. La prossima major (PPC 5.x) porterà, sulla
...
→ Vedi `docs/PPC_5x_ROADMAP.md` per il piano d'azione completo.
