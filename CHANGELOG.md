...
## v5.1.0 — Rebrand PPC + OCR IT/EN (2026-07-25)

**Nuova identità.** Il progetto non è più "PPC — Programming Cycling Coach" (fork anonimo di PPC): ora si chiama **PPC — Adaptive Cycling Intelligence**.
- **Nome:** PPC = VELocità + ARCO di potenza. Evoca la *power-duration curve*, il cuore scientifico del pianificatore.
- **Logo:** arco di potenza ascendente che diventa una ruota, gradiente teal→amber (palette PPC). File vettoriale `assets/ppc_logo.svg` + PNG/ICO per EXE/installer.
- **Dove appare:** titolo finestra/EXE (`PPC.exe`), FastAPI title, dashboard header + riepilogo, installer NSIS (nome + scorciatoie + icona), `ppc.spec` (icona + bundle identifier `com.ppc.cycling`), README.
...
- Under the hood: PPC can now compare a prescribed workout against the power you actually delivered, rep by rep — groundwork for judging whether a session was really completed, not just whether the total looked right.
...
- **Mid-plan entry — "I've been training already."** When you build or rebuild a
  plan you can now tell PPC you didn't start today, two ways:
  ...
  - **Place me from my rides** — PPC reads your recent training and
...
- **Your FTP is read from the effort itself.** When you finish a test, PPC
...
- **Short intervals capped to your measured power (opt-in).** If you have a
  measured peak-power number, PPC can hold a workout's short, very-hard
...
- Safe by design: PPC only ever touches the calendar entries it created
  — your races, notes, and anything you added yourself stay put, even on the
  same day. Turning sync off (or disconnecting) removes PPC's entries
  and nothing else. One PPC install per athlete.
...
- **Logs no longer balloon** — PPC now keeps a single small capped log
...
PPC re-optimizes your plan after you miss a session or sync new rides. Two
...
- **Polarization label now matches intervals.icu.** A ride could read "Unique" in
  PPC while intervals.icu called it "Polarized" — the classifier evaluated an
...
All workouts are `<author>PPC Library</author>`, content-derived generic
...
| | v1.8.6 | v1.8.7 |
|---|---|---|
| Banner header | "PPC v1.8.7 available" | "Update available — v1.8.6 → v1.8.7" |
...
Maintainer enrolled in Apple Developer Program (team `VB8TF5LQ8P`). v1.8.5 is the first PPC release codesigned with `Developer ID Application: Martijn Haring (VB8TF5LQ8P)` AND notarized through Apple's malware scan AND stapled offline.
...
| Drag .app to Applications | "PPC is damaged" → `xattr` Terminal fix | Launches on double-click |
...
Notarized DMG at `~/Desktop/PPC.dmg` → `spctl --assess` reports `source=Notarized Developer ID`. Both the .app and the DMG carry stapled tickets (offline-validatable).

## v1.8.4 — Ad-hoc codesigned DMG (no more "damaged" dialog) (2026-05-19)

User reported: downloaded DMG from GitHub releases triggered macOS Gatekeeper's "PPC is damaged and can't be opened. You should move it to the Bin." dialog — the misleading message panics most users into deleting the app. Locally-built DMG worked fine (no browser-quarantine flag attached).
...
```bash
codesign --force --deep --options runtime --sign - dist/PPC.app
codesign --force --sign - ~/Desktop/PPC.dmg
```
...
What changes for users:
- First launch shows the milder **"PPC cannot be opened because it is from an unidentified developer"** dialog instead of the alarming "damaged" message.
...
- After every ICU sync, when a ride lands with `dfa_alpha1_status == 'no_rr_data'`, PPC surfaces a one-time-per-version toast.
...
- **Upgrade docs** — new `docs/upgrading.md` (≤400 words) + new `## Updating PPC` README section. Per-data-type preservation table guarantees rider state lives in `~/.domestique/` outside the app bundle, so DMG / EXE replacement never touches profiles, rides, plans, FTP history, wellness logs, or ICU credentials.
...
PPC reaches public availability. The codebase has been in private development since 2026 — the iteration history is preserved below as "pre-release development log" so users can see the path from concept to v1.0.
...
- All ZWO files now have `<author>PPC Library</author>`.
...
- **ICU sync was silently 403'd by Cloudflare** for the default Python-urllib User-Agent. Affected all `urllib.request.urlopen` calls to intervals.icu since at least v4.4.0. Symptom: "Connect Intervals.icu in Settings" toast even after pasting valid creds. Now sends `User-Agent: PPC/4.5.4 (https://github.com/platypus45/domestique)` on every request → all calls succeed.
...
### BREAKING
- Trainer hardware support removed entirely. PPC is now a planner + workout library + post-ride viewer. Ride on Tacx app / MyWhoosh / Golden Cheetah / Zwift, import FIT after.
...
### Ship
DMG rebuilt to `~/Desktop/PPC.dmg`. `/Applications/PPC.app` not touched.
...
### Ship constraint
DMG rebuilt to `~/Desktop/PPC.dmg` only. `/Applications/PPC.app` not touched.
...
### Ship constraint
DMG rebuilt to `~/Desktop/PPC.dmg` only. **`/Applications/PPC.app` not touched** — user installs the DMG themselves after the next ride.
...
### Ship constraint
- DMG rebuilt to `~/Desktop/PPC.dmg`. `/Applications/PPC.app` NOT touched (user still testing on the bike).
...
### Ship constraint
- DMG rebuilt to `~/Desktop/PPC.dmg` only. **Installed app at `/Applications/PPC.app` was NOT touched** — user was testing on the bike during the wave; they install the new DMG themselves after the ride.
...
- `SCIENCE_REVIEW.md` title now "PPC".
...
### Added
- **Strava OAuth relay (Cloudflare Worker)** — true reference-parity UX: end-users no longer need to paste a Strava `client_secret`. A tiny single-file Worker (`relay/strava-oauth-worker.js`) holds the secret server-side and exposes `POST /token-exchange`, `POST /token-refresh`, and `GET /health`. CORS is locked to PPC's `localhost:8080` / `127.0.0.1:8080` origins; input shape (code/refresh_token) is validated at the edge; per-(colo,IP) token-bucket rate limit; logs contain only timestamps + status codes (never the secret, never any token).
...
### Renamed
- **ChickenCycling → PPC.** New tagline: **"Your training domestique."**
- macOS bundle id: `com.chickencycling.app` → `com.platypus45.domestique`
- DMG output: `ChickenCycling.dmg` → `PPC.dmg`
- PyInstaller spec: `chickencycling.spec` → `domestique.spec`
- Window title, tray-icon title, and printed banners now read "PPC"
...
- **Strava attribution.** To get the "via PPC" line on uploaded activities, register PPC with `developers@strava.com` (FIT manufacturer/product registration). Until then uploads land without a third-party attribution badge but otherwise function normally.
- DB table names, route IDs, intervals.icu credential helpers, and Python logger names are unchanged.
- Existing `<author>ChickenCycling</author>` headers inside bundled `workouts/*.zwo` and `courses/**/*.crs` are preserved as historical artifacts; only newly generated files emit `PPC`.
