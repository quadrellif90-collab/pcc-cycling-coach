...
- **Nome:** PCC = VELocità + ARCO di potenza. Evoca la *power-duration curve*, il cuore scientifico del pianificatore.
- **Logo:** arco di potenza ascendente che diventa una ruota, gradiente teal→amber (palette PCC). File vettoriale `assets/ppc_logo.svg` + PNG/ICO per EXE/installer.
- **Dove appare:** titolo finestra/EXE (`PCC.exe`), FastAPI title, dashboard header + riepilogo, installer NSIS (nome + scorciatoie + icona), `pcc.spec` (icona + bundle identifier `com.pcc.cycling`), README.
...
- i nomi hardcoded `PCC-Setup.exe`/`PCC.dmg` corretti in `PCC-Setup.exe`/`PCC.dmg`.
- Quindi: controllo automatico ogni 6h + installazione con un click, dati utente preservati.

**Sotto il cofano.** Rebrand + layer + auto-update in `app.py`, `pcc.spec`, `installer.nsi`, `build_win.bat`, `training_planner.py` (layer + `altitude_note` su `PlannedSession`), `plan_options.py` (`enable_altitude`), `templates/dashboard.html` (toggle + bottone aggiorna). Test: `tests/test_plan_options_51.py` (altitude + iniezione strength/mobility + non-regressione). Build: installer NSIS `PCC-Setup-<ver>.exe` + DMG `PCC.app`.
...
- Build: `pytesseract` + `PyMuPDF` in requirements; hidden imports in pcc.spec.
...
- Under the hood: PCC can now compare a prescribed workout against the power you actually delivered, rep by rep — groundwork for judging whether a session was really completed, not just whether the total looked right.
...
- **Mid-plan entry — "I've been training already."** When you build or rebuild a
  plan you can now tell PCC you didn't start today, two ways:
  ...
  - **Place me from my rides** — PCC reads your recent training and
...
- **Your FTP is read from the effort itself.** When you finish a test, PCC
...
- **Short intervals capped to your measured power (opt-in).** If you have a
  measured peak-power number, PCC can hold a workout's short, very-hard
...
- Safe by design: PCC only ever touches the calendar entries it created
  — your races, notes, and anything you added yourself stay put, even on the
  same day. Turning sync off (or disconnecting) removes PCC's entries
  and nothing else. One PCC install per athlete.
...
- **Logs no longer balloon** — PCC now keeps a single small capped log
...
PCC re-optimizes your plan after you miss a session or sync new rides. Two
...
- **Polarization label now matches intervals.icu.** A ride could read "Unique" in
  PCC while intervals.icu called it "Polarized" — the classifier evaluated an
...
All workouts are `<author>PCC Library</author>`, content-derived generic
...
| | v1.8.6 | v1.8.7 |
|---|---|---|
| Banner header | "PCC v1.8.7 available" | "Update available — v1.8.6 → v1.8.7" |
...
Maintainer enrolled in Apple Developer Program (team `VB8TF5LQ8P`). v1.8.5 is the first PCC release codesigned with `Developer ID Application: Martijn Haring (VB8TF5LQ8P)` AND notarized through Apple's malware scan AND stapled offline.
...
| Drag .app to Applications | "PCC is damaged" → `xattr` Terminal fix | Launches on double-click |
...
Notarized DMG at `~/Desktop/PCC.dmg` → `spctl --assess` reports `source=Notarized Developer ID`. Both the .app and the DMG carry stapled tickets (offline-validatable).

## v1.8.4 — Ad-hoc codesigned DMG (no more "damaged" dialog) (2026-05-19)

User reported: downloaded DMG from GitHub releases triggered macOS Gatekeeper's "PCC is damaged and can't be opened. You should move it to the Bin." dialog — the misleading message panics most users into deleting the app. Locally-built DMG worked fine (no browser-quarantine flag attached).
...
```bash
codesign --force --deep --options runtime --sign - dist/PCC.app
codesign --force --sign - ~/Desktop/PCC.dmg
```
...
What changes for users:
- First launch shows the milder **"PCC cannot be opened because it is from an unidentified developer"** dialog instead of the alarming "damaged" message.
...
- After every ICU sync, when a ride lands with `dfa_alpha1_status == 'no_rr_data'`, PCC surfaces a one-time-per-version toast.
...
- **Upgrade docs** — new `docs/upgrading.md` (≤400 words) + new `## Updating PCC` README section. Per-data-type preservation table guarantees rider state lives in `~/.domestique/` outside the app bundle, so DMG / EXE replacement never touches profiles, rides, plans, FTP history, wellness logs, or ICU credentials.
...
PCC reaches public availability. The codebase has been in private development since 2026 — the iteration history is preserved below as "pre-release development log" so users can see the path from concept to v1.0.
...
- All ZWO files now have `<author>PCC Library</author>`.
...
- **ICU sync was silently 403'd by Cloudflare** for the default Python-urllib User-Agent. Affected all `urllib.request.urlopen` calls to intervals.icu since at least v4.4.0. Symptom: "Connect Intervals.icu in Settings" toast even after pasting valid creds. Now sends `User-Agent: PCC/4.5.4 (https://github.com/platypus45/domestique)` on every request → all calls succeed.
...
### BREAKING
- Trainer hardware support removed entirely. PCC is now a planner + workout library + post-ride viewer. Ride on Tacx app / MyWhoosh / Golden Cheetah / Zwift, import FIT after.
...
### Ship
DMG rebuilt to `~/Desktop/PCC.dmg`. `/Applications/PCC.app` not touched.
...
### Ship constraint
DMG rebuilt to `~/Desktop/PCC.dmg` only. `/Applications/PCC.app` not touched.
...
### Ship constraint
DMG rebuilt to `~/Desktop/PCC.dmg` only. **`/Applications/PCC.app` not touched** — user installs the DMG themselves after the next ride.
...
### Ship constraint
- DMG rebuilt to `~/Desktop/PCC.dmg`. `/Applications/PCC.app` NOT touched (user still testing on the bike).
...
### Ship constraint
- DMG rebuilt to `~/Desktop/PCC.dmg` only. **Installed app at `/Applications/PCC.app` was NOT touched** — user was testing on the bike during the wave; they install the new DMG themselves after the ride.
...
- `SCIENCE_REVIEW.md` title now "PCC".
...
### Added
- **Strava OAuth relay (Cloudflare Worker)** — true reference-parity UX: end-users no longer need to paste a Strava `client_secret`. A tiny single-file Worker (`relay/strava-oauth-worker.js`) holds the secret server-side and exposes `POST /token-exchange`, `POST /token-refresh`, and `GET /health`. CORS is locked to PCC's `localhost:8080` / `127.0.0.1:8080` origins; input shape (code/refresh_token) is validated at the edge; per-(colo,IP) token-bucket rate limit; logs contain only timestamps + status codes (never the secret, never any token).
...
### Renamed
- **ChickenCycling → PCC.** New tagline: **"Your training domestique."**
- macOS bundle id: `com.chickencycling.app` → `com.platypus45.domestique`
- DMG output: `ChickenCycling.dmg` → `PCC.dmg`
- PyInstaller spec: `chickencycling.spec` → `domestique.spec`
- Window title, tray-icon title, and printed banners now read "PCC"
...
- **Strava attribution.** To get the "via PCC" line on uploaded activities, register PCC with `developers@strava.com` (FIT manufacturer/product registration). Until then uploads land without a third-party attribution badge but otherwise function normally.
- DB table names, route IDs, intervals.icu credential helpers, and Python logger names are unchanged.
- Existing `<author>ChickenCycling</author>` headers inside bundled `workouts/*.zwo` and `courses/**/*.crs` are preserved as historical artifacts; only newly generated files emit `PCC`.
