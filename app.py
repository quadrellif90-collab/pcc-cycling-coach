"""
PCC Dashboard v2 — local web interface.
...
import error_codes
import plan_options as PO          # PCC 5.x — PlanOptions selector (leaf, stdlib)
_DIAG_RING_MAX = 256
...
# indoor app (Tacx / MyWhoosh / Golden Cheetah), and PCC imports
...
def _select_platform_asset(assets, plat):
    ...
    macOS (`darwin`) → `.dmg`, prefer plain `PCC.dmg` over decorated
    variants like `PCC-1.0.3.dmg` so the canonical asset wins.
...
def api_update_check(force: int = Query(0)):
    ...
    try:
        ...
        payload = {
            ...
            "release_url": rel.get("html_url") or ("https://github.com/quadrellif90-collab/pcc-cycling-coach/releases/tag/v" + tag),
...
async def api_self_update(request: Request):
    ...
    installa:
      - Windows: esegue l'installer NSIS silenzioso (PCC-Setup.exe /S) che
        sostituisce l'EXE; poi termina l'app per lasciare libero il file.
      - macOS: monta PCC.dmg e copia l'app in /Applications (cp -R), poi
...
def api_weekly_plan(week_offset: int = Query(0)):
    ...
    def _session_out(s):
        ...
        out = {
            ...
            # PCC 5.x — accorgimenti layer notes (empty string when layer off).
...
async def api_plan_generate(request: Request):
    ...
    try:
        ...
        # PCC 5.x — accorgimenti selector. The UI sends a "plan_options" object
                    ...
                    "sessions": [
                        {
                            ...
                            # PCC 5.x — accorgimenti layer notes (empty when off).
                            "nutrition_note": getattr(s, "nutrition_note", ""),
                            "integrator_note": getattr(s, "integrator_note", ""),
                            "heat_note": getattr(s, "heat_note", ""),
                            "strength_note": getattr(s, "strength_note", ""),
...
def _build_fit_workout(name: str, blocks: list[dict], ftp: int,
    ...
    # unregistered third-party app like PCC. Using a real brand's id
...
def _is_cycling_sport(sport) -> bool:
    ...
    often carry no sport tag, and PCC is a cycling app). A clearly
...
# recorded outside PCC (Tacx / MyWhoosh / Golden Cheetah / any FIT
...
# actual power. PCC used to display the label verbatim, so the user
...
def _diag_local_only(request: Request) -> bool:
    """True iff the request is from localhost. PCC listens only on
...
if __name__ == "__main__":
    ...
    assert _UVICORN_WORKERS == 1, (
        "PCC requires single-worker uvicorn — see launcher.py / README "
