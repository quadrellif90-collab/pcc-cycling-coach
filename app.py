"""
PPC Dashboard v2 — local web interface.
...
# indoor app (Tacx / MyWhoosh / Golden Cheetah), and PPC imports
...
app = FastAPI(title="PPC — Adaptive Cycling Intelligence", version=_VERSION, lifespan=lifespan)
...
def api_my_calendar():
    ...
    vs eseguito. Legge il piano di PPC (tss_target) e le attività reali
...
def api_my_push_plan():
    ...
    for wk in plan_weeks:
        ...
        desc = f"TSS target {round(wk.get('tss_target',0))} · piano PPC (ciclismo+forza+mobilità+nutrizione)"
...
def _select_platform_asset(assets, plat):
    ...
    macOS (`darwin`) → `.dmg`, prefer plain `PPC.dmg` over decorated
    variants like `PPC-1.0.3.dmg` so the canonical asset wins.
...
def _build_fit_workout(name: str, blocks: list[dict], ftp: int,
    ...
    # unregistered third-party app like PPC. Using a real brand's id
...
def _is_cycling_sport(sport) -> bool:
    ...
    often carry no sport tag, and PPC is a cycling app). A clearly
...
# recorded outside PPC (Tacx / MyWhoosh / Golden Cheetah / any FIT
...
# actual power. PPC used to display the label verbatim, so the user
...
def _diag_local_only(request: Request) -> bool:
    """True iff the request is from localhost. PPC listens only on
...
if __name__ == "__main__":
    ...
    assert _UVICORN_WORKERS == 1, (
        "PPC requires single-worker uvicorn — see launcher.py / README "
        "for the reason (per-process caches, DB sync thread)."
    )
    print("PPC Dashboard - http://localhost:8080")
